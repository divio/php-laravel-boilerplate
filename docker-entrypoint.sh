...
# change app settings
sed -i "s/APP_URL=.*/APP_URL=https:\/\/${DOMAIN:-localhost}/g" /code/.env
sed -i "s/APP_DEBUG=.*/APP_DEBUG=${DEBUG:-true}/g" /code/.env
...
sed -i "s/DB_HOST=.*/DB_HOST=${DB_HOST/\:$DB_PORT/}/g" /code/.env
sed -i "s/DB_PORT=.*/DB_PORT=${DB_PORT:-5432}/g" /code/.env
sed -i "s/DB_DATABASE=.*/DB_DATABASE=${DB_DATABASE:-db}/g" /code/.env
sed -i "s/DB_USERNAME=.*/DB_USERNAME=${DB_USERNAME:-postgres}/g" /code/.env
sed -i "s/DB_PASSWORD=.*/DB_PASSWORD=${DB_PASSWORD}/g" /code/.env

# force set drivers
sed -i "s/CACHE_DRIVER=.*/DB_DATABASE=database/g" /code/.env
sed -i "s/SESSION_DRIVER=.*/DB_USERNAME=database/g" /code/.env
...
if [ -n "$DEFAULT_STORAGE_DSN" ]; then 
    ...
    sed -i "s/FILESYSTEM_DRIVER=.*/FILESYSTEM_DRIVER=s3/g" /code/.env
    sed -i "s/AWS_ACCESS_KEY_ID=.*/AWS_ACCESS_KEY_ID=$AWS_USERNAME/g" /code/.env
    sed -i "s/AWS_SECRET_ACCESS_KEY=.*/AWS_SECRET_ACCESS_KEY=$AWS_PASSWORD/g" /code/.env
    sed -i "s/AWS_BUCKET=.*/AWS_BUCKET=$AWS_HOST/g" /code/.env
